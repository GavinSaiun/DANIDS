...
def run_seed(
    ...
    for method, replay_budget in conditions:
        ...
        #
        # FAIRNESS CONTROL:
        # For a given seed + replay budget, Random and Boundary replay use
...
def plot_method_comparison(
    ...
    savefig(output_dir / "boundary_vs_random_source_auroc.png")
    ...
    plt.ylim(0.85, 1.01)
    plt.legend(frameon=False)
    savefig(output_dir / "boundary_vs_random_target_auroc.png")
    ...
    plt.legend(frameon=False)
    savefig(output_dir / "boundary_vs_random_forgetting.png")
...
def main() -> None:
    ...
    output_dir = (
        pair_dir
        / "mlp_boundary_replay_multiseed_results"
    )
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    ...
    details = {
        ...
        "research_question": (
            "Can uncertainty/boundary-based source replay preserve "
            "previous-domain performance more efficiently than random replay "
            "at the same replay-memory budget?"
        ),
        "protocol": {
            "within_seed": (
    ...
    raw_csv = (
        output_dir
        / "boundary_vs_random_raw_results.csv"
    )
    aggregate_csv = (
        output_dir
        / "boundary_vs_random_aggregate_summary.csv"
    )
    json_path = (
        output_dir
        / "boundary_vs_random_detailed_results.json"
    )
    ...
    print("\n" + "=" * 100)
    print("BOUNDARY VS RANDOM REPLAY COMPLETE")
    print("=" * 100)
    ...
    print("- boundary_vs_random_source_auroc.png")
    print("- boundary_vs_random_target_auroc.png")
    print("- boundary_vs_random_forgetting.png")
